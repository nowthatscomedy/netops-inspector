...
엑셀/CSV/JSON 장비 인벤토리를 읽고 SSH/Telnet으로 접속하여 점검 명령을 실행하고, 출력을 파싱해 결과 파일(Excel/JSON/CSV)을 생성합니다.
...
- 실행 중 실시간 터미널 대시보드
- 장비별 세션 로그 파일
- 컬럼 별칭/순서 설정 가능한 결과 파일(Excel/JSON/CSV) 생성
...
- 플러그인 플랫폼 초안: `docs/plugin-platform-draft.md`
