...
- **CLI UX 강화**: ASCII 배너(모든 메뉴 상단)와 박스형 메뉴 UI, 컬러 로그, 진행률 바 등으로 실행 상태를 가독성 있게 표시합니다. 콘솔 로그 레벨과 무관하게 진행 상황(연결 테스트, 명령 실행, 백업 저장 등)은 항상 출력되며, 쓰레드명은 `Thread-<인덱스>` 형식으로 표시됩니다.
