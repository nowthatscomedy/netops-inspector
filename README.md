...
- **병렬 처리**: 다수의 장비를 동시에 점검하며, '점검+백업' 모드에서는 장비별 점검/백업을 분리 스레드로 병렬 실행합니다.
- **자동 결과 리포트**: 점검 결과를 `results/inspection_results_...xlsx` 파일로 자동 생성하며, 접속 실패 항목은 하이라이트 처리됩니다.
- **CLI UX 강화**: ASCII 배너(모든 메뉴 상단)와 박스형 메뉴 UI, 컬러 로그, 진행률 바 등으로 실행 상태를 가독성 있게 표시합니다. 콘솔 로그 레벨과 무관하게 진행 상황(연결 테스트, 명령 실행, 백업 저장 등)은 항상 출력되며, 쓰레드명은 `Device-<인덱스>`(상위), `Device-<인덱스>:Inspect`, `Device-<인덱스>:Backup` 형식으로 표시됩니다.
- **상세 로그**: 모든 작업 과정과 장비와의 통신 내용을 세션별 로그 파일로 기록하며, 점검/백업 병렬 실행 시 세션 로그가 분리 저장됩니다.
...
`settings.json`은 콘솔 로그 출력 수준을 저장합니다. 파일이 없으면 기본값(`WARNING`)으로 시작하며, 메뉴에서 변경 시 자동으로 생성/갱신됩니다.
